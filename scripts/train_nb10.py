...
CONTINENT_MAP = {
    ...
    "Comoros": "Afrique",
    "Congo": "Afrique",
    "Costa Rica": "Amerique centrale",
    "Croatia": "Europe",
    "Cuba": "Caraibes",
    "Cyprus": "Europe",
    "Czechia": "Europe",
    "Denmark": "Europe",
    "Democratic Republic of Congo": "Afrique",
    "Djibouti": "Afrique",
    "Dominican Republic": "Caraibes",
    "East Timor": "Asie",
    "Ecuador": "Amerique du Sud",
    "Egypt": "Afrique",
    "El Salvador": "Amerique centrale",
    "Eritrea": "Afrique",
    "Estonia": "Europe",
    "Eswatini": "Afrique",
    "Ethiopia": "Afrique",
...
def entrainer_et_exporter(df: pd.DataFrame) -> None:
    ...
    logger.info(
        "Train : %d lignes | Test : %d lignes | %d features",
        len(X_train),
        len(X_test),
        X_train.shape[1],
    )
    ...
    if HAS_MLFLOW:
        tracking_uri = (
            ...
            or f"sqlite:///{(BASE / 'mlflow.db').resolve()}"
        )
    ...
    with parent_cm as parent_run:
        ...
        for name, spec in REG_MODELS.items():
            ...
            logger.info(
                "REG %s : R2_test=%.4f MAE=%.4f (%.1fs)", name, r2_te, mae_te, dur
            )
            ...
            if HAS_MLFLOW:
                ...
                with mlflow.start_run(run_name=f"reg_{name}", nested=True):
                    ...
                    mlflow.log_params(
                        {
                            "model_type": spec["class"].__name__,
                            "task": "regression",
                            **spec["kwargs"],
                        }
                    )
        ...
        for name, spec in CLS_MODELS.items():
            ...
            logger.info(
                "CLS %s : acc_test=%.4f f1_macro=%.4f (%.1fs)", name, acc_te, f1_te, dur
            )
        ...
        if HAS_MLFLOW:
            ...
            mlflow.sklearn.log_model(
                ...
                registered_model_name=(
                    "NB10_classification" if env_label != "ci" else None
                ),
            )
            ...
            for png in [
                "NB10_eda.png",
                "NB10_regression.png",
                "NB10_classification.png",
                "NB10_predictions_2030.png",
            ]:
                p = OUTPUT_DIR / png
                if p.exists():
            ...
            # Tableau comparatif
            pd.DataFrame(results_reg).to_csv(
                OUTPUT_DIR / "NB10_mlflow_reg_grid.csv", index=False
            )
            pd.DataFrame(results_cls).to_csv(
                OUTPUT_DIR / "NB10_mlflow_cls_grid.csv", index=False
            )
            mlflow.log_artifact(str(OUTPUT_DIR / "NB10_mlflow_reg_grid.csv"))
            mlflow.log_artifact(str(OUTPUT_DIR / "NB10_mlflow_cls_grid.csv"))

            logger.info(
                "MLflow : %d runs reg + %d runs cls + parent sur %s",
                len(results_reg),
                len(results_cls),
                tracking_uri,
            )
    ...
    for _, row in df_2020.iterrows():
        ...
        hist_2010 = df_2010[
            (df_2010["country"] == pays) & (df_2010["disaster_type"] == typ)
        ]
    ...
    df_country = (
        df_pred.groupby(["country", "continent"])["impact_pred"].sum().reset_index()
    )
    ...
    ].to_csv(OUTPUT_DIR / "NB10_predictions_2030_detail.csv", index=False)
